...
export async function readRuntimeConfig(): Promise<RuntimeConfig> {
    ...
    return {
        openaiApiKey: emptyToNull(configRow?.openaiApiKey ?? null),
        creatomateApiKey: emptyToNull(configRow?.creatomateApiKey ?? null),
        creatomateWebhookSecret: emptyToNull(configRow?.creatomateWebhookSecret ?? null),
        elevenlabsVoiceId: emptyToNull(configRow?.elevenlabsVoiceId ?? null),
        elevenlabsModelId: emptyToNull(configRow?.elevenlabsModelId ?? null),
        renderWebhookUrl: emptyToNull(configRow?.renderWebhookUrl ?? null),
        creditsAvailable: clampCredits(configRow?.creditsAvailable),
    };
}
...
export async function decrementCredits(amount = 1): Promise<number | null> {
    ...
    const nextValue = Math.max(0, clampCredits(existing.creditsAvailable) - Math.max(0, amount));
    await prisma.appConfig.update({
        where: { id: existing.id },
...
export async function getEffectiveConfig(): Promise<RuntimeConfig> {
    ...
    return {
        ...
        creatomateApiKey: emptyToNull(process.env.CREATOMATE_API_KEY) ?? stored.creatomateApiKey,
        creatomateWebhookSecret:
            emptyToNull(process.env.CREATOMATE_WEBHOOK_SECRET) ?? stored.creatomateWebhookSecret,
        elevenlabsVoiceId: emptyToNull(process.env.ELEVENLABS_VOICE_ID) ?? stored.elevenlabsVoiceId,
        elevenlabsModelId: emptyToNull(process.env.ELEVENLABS_MODEL_ID) ?? stored.elevenlabsModelId,
        renderWebhookUrl: emptyToNull(process.env.RENDER_WEBHOOK_URL) ?? stored.renderWebhookUrl,
